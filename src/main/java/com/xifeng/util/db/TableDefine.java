	...
	public static List<Table> getTableCols(Connection conn, String dbName,String[] tables) {
			...
			try {
				...
				for (String table : tables) {
					ResultSet resultSet = dbmd.getTables(dbName, "%",
					...
					while (resultSet.next()) {
						...
						while (rs.next()) {
							...
							System.out.println(colName + "----" + typeName + "--"
									+ isNull + "---" + marks + "----" + precision);
							TableColumn tableColumn = new TableColumn(colName,typeName,colSize,isNull,precision,columnDef,marks);
							tableColumnList.add(tableColumn);
						}
					}
					System.out.println("++-------------------------++");
					System.out.println("export table target :"+tables.length+",success:"+count);
					tableList.add(new Table(table,tableColumnList));
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
